...
void multiply1(DataSet dataSet, int numThreads);
void multiply2(DataSet dataSet, int numThreads);
...
int main(int argc, char *argv[]) {
	...
	for (int co = 0; co < 4; co++) {
		...
		//multiply(dataSet);
		multiply1(dataSet, thread);

		elapsedtime = omp_get_wtime() - starttime;

		printf("Time Elapsed with %d thread is  %f mSecs  \n", thread, elapsedtime * 100);
...
void multiply1(DataSet dataSet, int numThreads) {
	int i, j, k, sum;
#pragma omp parallel for num_threads(numThreads) private( j, sum, k ) 
	for (i = 0; i < dataSet.n; i++) {
		for (j = 0; j < dataSet.p; j++) {
			sum = 0;
			for (k = 0; k < dataSet.m; k++) {
...
void multiply2(DataSet dataSet, int numThreads) {
	int i, j, k, sum = 0;
#pragma omp parallel for collapse(2) num_threads(numThreads) private( k ,sum) 
	for (i = 0; i < dataSet.n; i++) {
		for (j = 0; j < dataSet.p; j++) {
			sum = 0;
			for (k = 0; k < dataSet.m; k++) {
